 ...
 */
public class ControllerCodeGenerator extends AbstractCodeGenerator<ControllerCodegenConfiguration> {

	private static final Logger LOGGER = LoggerFactory.getLogger(ControllerCodeGenerator.class);
	...
	public ControllerCodeGenerator(String businessModule, Class<?>... modelClasses) {
	 ...
	 * @return
	 */
	public ControllerCodeGenerator overrideCodegenConfig(Consumer<ControllerCodegenConfigProperties> override) {
	...
	protected String generateControllerCode(Class<?> modelClass) throws Exception {
		...
		String generatedFilePath = doControllerCodeGeneration(controllerCodeParameter);
		LOGGER.info(">>> 生成数据模型[{}]的{}代码 : {}", modelClass.getName(), getCodeModule().getName(), generatedFilePath);
	...
	protected File getControllerCodeTemplateDir() {
		return new File(ControllerCodeGenerator.class.getResource("").getFile());
	}
