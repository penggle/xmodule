...
public class ServiceCodeGenerator extends AbstractCodeGenerator<ServiceCodegenConfiguration> {
	...
	public ServiceCodeGenerator(String businessModule, Class<?>... modelClasses) {
	 ...
	 * @return
	 */
	public ServiceCodeGenerator overrideCodegenConfig(Consumer<ServiceCodegenConfigProperties> override) {
	...
	protected String generateServiceInterfaceCode(Class<?> modelClass) throws Exception {
		...
		String generatedFilePath = doServiceInterfaceCodeGeneration(serviceCodeParameter);
		LOGGER.info(">>> 生成数据模型[{}]的{}接口代码 : {}", modelClass.getName(), getCodeModule().getName(), generatedFilePath);
	...
	protected String generateServiceImplementCode(Class<?> modelClass) throws Exception {
		...
		String generatedFilePath = doServiceImplementCodeGeneration(serviceCodeParameter);
		LOGGER.info(">>> 生成数据模型[{}]的{}实现代码 : {}", modelClass.getName(), getCodeModule().getName(), generatedFilePath);
		return generatedFilePath;
	}
