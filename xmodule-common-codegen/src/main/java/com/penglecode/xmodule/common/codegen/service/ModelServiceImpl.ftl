...
public class ${serviceImplClassName} implements ${serviceClassName} {
	...
	@Override
	@Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
	public void delete${modelAliasName}ById(${modelIdClassName} ${modelIdName}) {
		ValidationAssert.notNull(${modelIdName}, "${modelIdName}不能为空");
		${modelClassNameLower}Mapper.deleteModelById(${modelIdName});
	}

	@Override
	public ${modelClassName} get${modelAliasName}ById(${modelIdClassName} ${modelIdName}) {
		return ModelDecodeUtils.decodeModel(${modelClassNameLower}Mapper.selectModelById(${modelIdName}));
	}
	...
	public List<${modelClassName}> get${modelAliasName}ListByPage(${modelClassName} condition, Page page, Sort sort) {
    	...
    	page.setTotalRowCount(${modelClassNameLower}Mapper.selectModelPageCountByExample(condition));
		return dataList;
	}
